...
type EnumNumber<T extends readonly string[], Result extends Record<string, string> = {}, Index extends any[] = []> = T extends readonly [infer F, ...infer R]
  ? R extends string[] ? F extends string ? EnumNumber<R, Copy<Result & {readonly [P in F as Capitalize<P>]: Index['length']}>, [...Index, F]> : never : never
  : Result;



type EnumString<T extends readonly string[], Result extends Record<string, string> = {}> = T extends readonly [infer F, ...infer R]
  ? R extends string[] ? F extends string ? EnumString<R, Result & {readonly [P in F as Capitalize<P>]: F}> : never : never
  : Result;


type r = Enum<typeof OperatingSystem>
