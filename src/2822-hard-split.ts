...
type StringToTuple<T extends string> = T extends `${infer F}${infer O}` ? [F, ...StringToTuple<O>] : [];


type Test<T extends string> = T extends `${infer F}${''}${infer O}` ? F : 2;

type r1 = Test<'wang z hel'>;

type Split<S extends string, SEP extends string> = [string] extends [S] ? string[]
  : SEP extends '' ? StringToTuple<S>
  : S extends `${infer F}${SEP}${infer O}` ? [F, ...Split<O, SEP>] : [S];

type r2 = '' extends  `${infer F}${infer O}` ? O : 2;

type r3 = Split<'aaaaaaaakaaaaaa', ''>;
