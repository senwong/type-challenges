...
type r1 = StringToUnion<''>;

type DropString<S, R extends string> = S extends `${infer F}${infer O}` ? `${F extends StringToUnion<R> ? '' : F}${DropString<O, R>}`  : '';

type r2 = DropString<' b u t t e r f l y ! ', 'but'>
